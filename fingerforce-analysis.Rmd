...
zz + 
    geom_point(data = nlsconf, aes(x = day + .15, y = meancoef),
               size = 4, color = 'black') + 
    geom_errorbar(data = nlsconf, 
                  aes(x = day + .15, y = NULL, ymax = mean95, ymin = mean05), 
                  colour = "black", size = 1, width = .1)
```

I couldn't get the new legend worked out, but the black is the estimate based on the profile confidence intervals from `nls`, and the colorful ones are the bootstrapped intervals from the `glm`.
